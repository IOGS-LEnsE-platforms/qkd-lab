...
# Note: please check the path of CPC.dll
#*********************************************************************
...
# Load library
def cpc_shared_lib(path):
  if platform.system() == "Windows":
      return WinDLL(path)
  elif platform.system() == "Linux":
      return CDLL("libCPC.so")
  elif platform.system() == "Darwin":
      return CDLL("libCPC.dylib")
  else:
      return Nones

# List devices
# Description: scan all CPC devices available
def listDevices(path):
  global DEVICE
  devList = []
  nDev = c_short()
  DEVICE = cpc_shared_lib(path)
...
# Open device
# Description: connect the target CPC device
def openDevice(iDev):
