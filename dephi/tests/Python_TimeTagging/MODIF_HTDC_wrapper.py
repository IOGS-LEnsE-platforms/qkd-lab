...
def htdc_shared_lib():
  if platform.system() == "Windows":
      return WinDLL(r"C:\Users\CRYPTO_B\Desktop\Docs IOGS QKD\Docs harware et API Aurea QKD\Chronoxea API\HTDC-API\Sources\bin\x64\HTDC.dll")
...
#Rajout du 21/02/2025 par Vincent PRADERE,
# permit to get the Version of the Librairy use to communicate with Correlator 
def getLibVersion():
    # Initialise un pointeur vers un entier de type c_ushort
    version = c_ushort()
    
    # Appelle la fonction qui récupère la version et stocke dans version
    ret = DEVICE.HTDC_getLibVersion(byref(version))
    
    print(f"Version brute: {hex(version.value)}")
    # Extraire la version majeure (MM) et mineure (mm) 
    major_version = (version.value >> 8) & 0xFF  # Décale de 8 bits et garde les 8 premiers bits
    minor_version = version.value & 0xFF  # Garde les 8 derniers bits
    
    # Retourner les valeurs
    return ret, f"Version: {major_version}.{minor_version}"


#Rajout du 21/02/2025 par Vincent PRADERE,
# permit to change the mode of transmission from the correlator 
#mode :
#0: HTDC raw data
#1: HTDC raw data + time tagging
#2: time tagging

def setResultFormat(iDev,iCh,mode):
    ret = DEVICE.HTDC_setResultFormat(iDev,iCh,mode)
    return ret
